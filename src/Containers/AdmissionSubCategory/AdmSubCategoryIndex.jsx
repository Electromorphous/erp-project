...
function AdmSubCategoryIndex() {
  ...
  const getData = async () => {
    let data = await Grid(
      "student/fetchAllFeeAdmissionSubCategoryDetail",
      "created_date"
    );
  ...
  const handleActive = async (params) => {
    ...
    let endPoint = "";
    if (check === true) {
      endPoint = "student/FeeAdmissionSubCategory";
    } else {
      endPoint = "student/ActivatefeeAdmissionSubCategory";
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Link to={`/AdmSubCategoryUpdate/${params.row.id}`}>
  ...
  return (
    <>
      <IndexCreateButton label="Create" path="/AdmSubCategoryCreation" />
