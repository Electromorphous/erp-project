...
function ProgramtypeIndex() {
  ...
  const getData = async () => {
    let data = await Grid("academic/fetchAllProgramTypeDetail", "created_date");
    setrows(data);
  };
  useEffect(() => {
    getData();
  ...
  const handleActive = async (params) => {
    ...
    let endPoint = "";
    if (check === true) {
      endPoint = "academic/ProgramType";
    } else {
      endPoint = "academic/activateProgramType";
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Link to={`/ProgramtypeUpdate/${params.row.id}`}>
  ...
  return (
    <>
      <IndexCreateButton label="Create" path="/ProgramtypeCreation" />
