...
function CurrencytypeIndex() {
  ...
  const getData = async () => {
    let data = await Grid("finance/fetchAllCurrencyTypeDetail", "created_date");
    setrows(data);
  };
  useEffect(() => {
    getData();
  ...
  const handleActive = async (params) => {
    ...
    let endPoint = "";
    if (check === true) {
      endPoint = "finance/CurrencyType";
    } else {
      endPoint = "finance/activeCurrencyType";
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Link to={`/CurrencytypeUpdate/${params.row.id}`}>
